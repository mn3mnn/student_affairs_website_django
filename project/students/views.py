    ...
    if request.method == 'POST':
        try:
            ...
            for obj in Student.objects.all():
                if obj.collegeID == id_of_college:
                    found_object = obj
                    break

            if not found_object:
                std = Student(name=name, collegeID=id_of_college, level=level, department=department, gender=gender,
...
def update_student_information(request):
    if request.method == 'POST':
        ...
        if id_of_collage:  # if the id is not None
            try:
                student = Student.objects.get(collegeID=id_of_collage) # throw exception if there is no student with this ID
    ...
    return render(request, 'students/updateStudentInformation.html')
...
def change_student_status(request):
    if request.method == 'POST':
        ...
        if id_of_college:
            std = Student.objects.get(collegeID=id_of_college)
            if std:
    ...
    return render(request, 'students/Change Student Status.html')
...
def assign_department(request):
    ...
    return render(request, 'students/assignDepartmentPage.html')
...
def search_for_students(request):
    ...
    return render(request, 'students/searchForStudents.html')
...
def student_info(request, std_id):
    try:
        student = Student.objects.get(collegeID=std_id)
        context = {'std': student}
